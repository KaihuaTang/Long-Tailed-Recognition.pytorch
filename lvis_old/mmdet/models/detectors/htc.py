...
class HybridTaskCascade(CascadeRCNN):
    ...
    def _bbox_forward_train(self,
        ...
        bbox_head.MU = 1.0 - (1 - 0.9) * 0.002
        cls_score, bbox_pred = bbox_head(bbox_feats, causal_infer, gt_label=bbox_targets[0])
    ...
    def _bbox_forward_test(self, stage, x, rois, semantic_feat=None, causal_infer=True):
        bbox_roi_extractor = self.bbox_roi_extractor[stage]
        bbox_head = self.bbox_head[stage]
        ...
        if self.with_semantic and 'bbox' in self.semantic_fusion:
            ...
            if bbox_semantic_feat.shape[-2:] != bbox_feats.shape[-2:]:
                bbox_semantic_feat = F.adaptive_avg_pool2d(
                    bbox_semantic_feat, bbox_feats.shape[-2:])
            bbox_feats += bbox_semantic_feat
        cls_score, bbox_pred = bbox_head(bbox_feats, causal_infer)
    ...
    def forward_dummy(self, img):
        ...
        for i in range(self.num_stages):
            cls_score, bbox_pred = self._bbox_forward_test(
                i, x, rois, semantic_feat=semantic_feat, causal_infer=True)
        ...
        for i in range(self.num_stages):
            ...
            loss_bbox, rois, bbox_targets, bbox_pred = \
                self._bbox_forward_train(
                    ...
                    rcnn_train_cfg, semantic_feat, causal_infer=True)
    ...
    def aug_test(self, imgs, img_metas, proposals=None, rescale=False):
        ...
        for x, img_meta, semantic in zip(
            ...
            for i in range(self.num_stages):
                bbox_head = self.bbox_head[i]
                cls_score, bbox_pred = self._bbox_forward_test(
                    i, x, rois, semantic_feat=semantic, causal_infer=True)
                ms_scores.append(cls_score)
