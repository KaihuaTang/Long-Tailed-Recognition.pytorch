...
class ConvFCBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        if self.with_cls:
            if self.DECONFOUND_TRAIN:
                self.fc_cls = nn.Parameter(torch.Tensor(self.num_classes, self.cls_last_dim).cuda(), requires_grad=True)
    ...
    def init_weights(self):
        # remove original by Kaihua
        return

    def forward(self, x, causal_infer=True, gt_label=None):
        # shared part
        ...
        # Look at here Kaihua Tang (used for global search)
        if self.DECONFOUND_TRAIN:
            cls_score = self.cos_forward(x_cls, self.CAUSAL_INFER, gt_label) if self.with_cls else None
        else:
    ...
    def cos_forward(self, x, causal_infer, gt_label):
        ...
        if (not self.training) and causal_infer:
            ...
            for nx, ne, nw in zip(x_list, e_list, w_list):
                ...
                y0 = torch.mm((1.5 * cos_val * ne) * self.SCALE, nw.t())
                output.append(y0)

            y0 = sum(output)
            #y0 = torch.mm(normed_e * self.SCALE, normed_w.t())
            
            if self.KEEP_FG:
                old_score = y.softmax(-1)
                new_score = (y - y0).softmax(-1)
                # keep the fg bg the same
                final_score_bg = old_score[:, 0].contiguous().view(-1, 1)
                final_score_fg = old_score[:, 1:].sum(-1, keepdim=True) / (new_score[:, 1:].sum(-1, keepdim=True) + 1e-10) * (new_score[:, 1:] + 1e-10)
                final_score = torch.cat([final_score_bg, final_score_fg], dim=-1)
                y = final_score
            else:
                y = y - y0
        return y
